...
export default function WieghtsTypeTable({
  receptionId,
  weightType,
  weightTypesdata,
}: {
  receptionId: number;
  weightType: string;
  weightTypesdata?: IReceptionWeightFish[];
}) {
  const { data, isPending, isSuccess } = useQuery({
    queryKey: ["WeightTypes"],
    queryFn: async () =>
      await getWeightsByTypesAndreception({ receptionId, type: weightType }),
              ...
              data?.map((item, index) => {
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-4">{item.crate}</td>
                    <td className="px-3 py-4">{formatFloat(item.weight)}{" "} kg</td>

                    <td className="px-3 py-4">
                      <Link
                        href={`/reception-weight-fish/${item.id}/update`}
