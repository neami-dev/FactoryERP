...
export default function FinishAddingTraceBtn({
  receptionId,
}: {
  receptionId: number;
}) {
  ...
  const finishReception = async () => {
    const updatedReception = await updateReception({
      reception: { id: receptionId, isFinished_add_trace: true,isFinished:true },
      path: ["/reception/history", "/traceability/reception"],
    });
