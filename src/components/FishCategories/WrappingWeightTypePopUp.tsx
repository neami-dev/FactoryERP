...
import { createWrappingWeightType, deleteWrappingWeighttype, isWrappingWeightFishUsed, updateWrappingWeightType, wrappingWeightTpeExists } from "@/lib/actions/wrappingWeightType.actions";
interface FormEvent {
  preventDefault: () => void;
}

export default function WrappingWeightTypePopUp({
  handleClick,
  fishCategoryId,
  weightTypeId,
  weightType,
  ...
  useEffect(() => {
    if (weightTypeId) {
      isWrappingWeightFishUsed(weightTypeId).then((res) => {
        setIsUsed(res);
      });
    }
  ...
  const handleDelete = async () => {
    if (weightTypeId) {
      deleteWrappingWeighttype(weightTypeId);
      handleClick(false);
      setFormData({ name: "" });
    }
  };
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (onCreate.success) {
      const exists = await wrappingWeightTpeExists({
    ...
    if (type === "Create") {
      ...
      const response = await createWrappingWeightType({
    ...
    if (type === "Update" && weightTypeId) {
      ...
      const response = await updateWrappingWeightType({
    ...
    <div className="w-[500px] h-[400px] absolute top-[130px] flex justify-center gap-8 flex-col item-center shadow-lg z-20 mx-auto rounded-md border bg-gray-50">
      <div className="relative">
        ...
        <h2 className="text-xl text-[#3354f4] font-semibold text-center py-4">
          {type === "Create" ? "Ajouter" : "Modifier"} le Type
        </h2>
      </div>
      <form
      ...
      >
        ...
        >
          {type === "Update" ? `Modifier le Poission` : "Créer le Poission"}
        </Button>
      </form>
    </div>
  );
